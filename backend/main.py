from fastapi import FastAPI, WebSocket, WebSocketDisconnect, Request, Response, HTTPException, UploadFile, File, Form
import shutil
...
try:
    ...
    class RevenueOrchestrator: 
        ...
        def calculate_split(self, r): return {"admin": r*self.platform_share, "uploader": r*self.creator_share, "viewer": r*self.user_share}
    class LiveMonitor:
    ...
    class DummyGovernor:
        ...
        async def verify_amarpay_payment(self, *args, **kwargs): return {"status": "ERROR"}
        async def verify_sslcommerz_payment(self, *args, **kwargs): return {"status": "ERROR"}
        def _save_processed_id(self, *args, **kwargs): pass
        def set_payout_mode(self, *args, **kwargs): pass
        async def execute_payout(self, *args, **kwargs): return {"status": "ERROR"}
    ...
    class DummyFinance:
        ...
        async def finalize_and_archive(self, *args, **kwargs): pass
        def _save_config(self, *args, **kwargs): pass
        def add_to_batch(self, *args, **kwargs): pass
    imperial_finance = DummyFinance()

# V15 Standard: Engine Ports & Gateways
PUBLIC_PORT = int(os.getenv("PUBLIC_PORT", 5000))
STREAM_PORT = int(os.getenv("STREAM_PORT", 8080))
STREAM_BASE = "media_stream" # Nginx Shard Base
...
# Sovereign V15: High-Speed Static Media Tunnel [A_113]
try:
    if not os.path.exists(SOV_DNA.storage):
        os.makedirs(SOV_DNA.storage, exist_ok=True)
    app.mount("/stream", StaticFiles(directory=SOV_DNA.storage), name="stream")
    app.mount("/media", StaticFiles(directory=SOV_DNA.storage), name="media")
except Exception as e:
    print(f"MEDIA_TUNNEL_ERROR: {e}")
...
class ConnectionManager:
    def __init__(self):
        ...
        self.ledger = {} # Sovereign V15: Active Financial Registry [A_113]
    ...
    async def sync_admin_tables(self, websocket: WebSocket):
        ...
        try:
            ...
            # 1.1 Gather Pending Verifications [A_107 Admin Pulse Fix]
            verification_list = []
            for m_id, v_data in id_vault.pending_verifications.items():
                v_entry = v_data.copy()
                if "doc_url" not in v_entry:
                     # Retroactive Healing for doc_url construction
                     v_entry["doc_url"] = f"http://{SOVEREIGN_HOST}:{PUBLIC_PORT}/{STREAM_BASE}/{v_entry.get('doc_path', 'ERROR')}"
                verification_list.append(v_entry)
...
async def register_media(reg: MediaRegistration):
    ...
    PROXY_PATH = STREAM_BASE # Unified V15 High-Speed Channel
    # Port 80/443 mapping doesn't need explicit ports in URL
    PORT_INFIX = "" if PROTOCOL == "https" else f":{STREAM_PORT}"
    ...
    entry = {
        ...
        "uploader_pic": manager._normalize_url(u_profile.get("profile_pic", "")) if u_profile else "",
        ...
        "saved_by": [],
        "comments_data": [],
        "is_verified": u_profile.get("is_verified", False) if u_profile else False,
        "analytics": {
            "watch_time_total": 0,
            "full_watches": 0,
            "territories": {"United States": 45, "United Kingdom": 20, "Mesh Network": 15}
        }
    }
...
class mlm_protocol:
    ...
    def __init__(self):
        ...
        self.bdt_rate = 115.0
        self.network_depth = 15 # V15 Standard Depth
        self.binding_otps = {} # mesh_id -> {otp, timestamp}
    ...
    def process_referral(self, referrer_id: str, new_user_id: str) -> dict:
        ...
        if fee > 0.0:
            current_bal = float(candidate_bal.get("USD", 0.0))
            if current_bal < fee:
                logger.warning(f"MLM_REJECT: {new_user_id} insufficient funds for activation (${fee})")
                return {"status": "REJECTED", "reason": f"INSUFFICIENT_FUNDS_FOR_ACTIVATION (Need ${fee})"}
            
            # Layer 2: Deduction
            candidate_bal["USD"] = round(current_bal - fee, 2)
            candidate_bal["signature"] = user_auth.sign_balance(
                new_user_id, candidate_bal["USD"], candidate_bal.get("BDT", 0.0), candidate_bal.get("COINS", 0)
            )
            logger.info(f"MLM: {new_user_id} charged ${fee} for activation via {referrer_id}.")
        else:
            logger.info(f"MLM: {new_user_id} activated instantly (Fee: $0.0) via {referrer_id}.")
            ...
            if auth_data.get("email_phone") == normalized_id:
                if auth_data.get("password") == hashed_pw:
                    ...
                    if sov_id in self.users:
                        ...
                        return {"status": "SUCCESS", "sov_id": sov_id, "name": u_data["name"], "token": token, "is_verified": u_data.get("is_verified", False)}
                else:
...
class identity_vault:
    ...
    def _load(self):
        if os.path.exists(self.db_file):
            try:
                ...
                print(f"IDENTITY_VAULT_LOAD_ERR: {e}")
        return {}
    ...
    def _save(self):
        try:
            ...
            print(f"IDENTITY_VAULT_SAVE_ERR: {e}")
        ...
        while True:
            ...
            try:
                ...
                if action == "AD_IMPRESSION":
                    ...
                    # Step 3: Split revenue based on dynamic MLM config (Admin Sliders)
                ...
                if action == "GET_INBOX":
                    ...
                    # V15 Dynamic Verification Sync for Inbox [A_120]
                    enriched_notifications = []
                    for n in notifications:
                        n_copy = n.copy()
                        from_id = n.get("from")
                        if from_id:
                           # Ensure we check the current status of the sender from User Auth
                           n_copy["is_verified"] = user_auth.users.get(from_id.upper(), {}).get("is_verified", False)
                ...
                if action == "USER_INTERACTION":
                                ...
                                if match:
                                    ...
                                    if i_type.startswith("VIDEO_UNLIKE"):
                                        ...
                                        if comment_text: 
                                            ...
                                            c_profile = user_auth.users.get(mesh_id, {})
                                            ...
                                            new_comment_obj = {
                                                ...
                                                "timestamp": datetime.datetime.now().isoformat(),
                                                "isVerified": c_profile.get("is_verified", False),
                    ...
                    for m in manager.media_registry:
                        ...
                        m_copy["is_verified"] = u_profile.get("is_verified", False)
                    ...
                    doc_url = f"http://{SOVEREIGN_HOST}:{PUBLIC_PORT}/{STREAM_BASE}/{doc_path}"
                    ...
                    await manager.send_personal_message(json.dumps({
                        ...
                        "risk": report["risk"],
                        "is_verified": user_auth.users[mesh_id].get("is_verified", False)
                    }), websocket)
                    ...
                    # Sovereign V15: Permanent Identity Sync [A_119 Master Parity]
                    ...
                    reason = payload.get("reason", "Inappropriate Ad")
                    reporter = mesh_id or "ANON_REPORTER"

                    logger.info(f"A_112 AD_REPORT: {ad_id} reported by {reporter}. Reason: {reason}")
                    ...
                    await manager.send_personal_message(json.dumps({
                        ...
                        "message": "SPONSORED NODE FLAGGED FOR REVIEW."
                    }), websocket)
    ...
    try:
        while True:
            ...
            if action == "GET_ALL_USERS":
                    ...
                    async with ledger_lock:
                        ...
                        for item in batch_res.get("details", []):
                            ...
                            if item.get("status") == "PAID":
                                ...
                                # 2. Sovereign V15: Final User Deduction Pulse
                                u_bal = manager.get_user_balance(u_id)
                                tax_amt = float(item.get("tax_rate", 0) / 100.0) * amount
                                ...
                                await manager.add_notification(
                                    ...
                                    {"msg": f"Withdrawal request of {amount} {currency} was REJECTED/FAILED.", "error": item.get("error")}
                                )
                ...
                mlm.platform_share = float(payload.get("p", (mlm.platform_share * 100.0))) / 100.0
                mlm.creator_share = float(payload.get("c", (mlm.creator_share * 100.0))) / 100.0
                mlm.user_share = float(payload.get("u", (mlm.user_share * 100.0))) / 100.0
                ...
                vault.ad_yield_string = payload.get("value", vault.ad_yield_string)
                logger.info(f"A_111: CPM Range Updated -> {vault.ad_yield_string}")
            elif payload.get("action") == "MLM_FEE_UPDATE":
                mlm.activation_fee = float(payload.get("value", str(mlm.activation_fee)).replace('$', '').strip())
                mlm._save_config()
            ...
            elif payload.get("action") == "A_107_VERIFICATION_DECISION" or action == "VERIFICATION_DECISION":
                u_id = payload.get("user_id") or payload.get("target_id")
                decision = payload.get("decision")
                
                # V15 Hardened Identity Sync: Persist decision to User Manifest
                if decision == "APPROVED":
                    if u_id in user_auth.users:
                        user_auth.users[u_id]["is_verified"] = True
                        user_auth._save_users()
                        logger.info(f"A_107 IDENTITY: User {u_id} VERIFIED by Admin.")
                
                # Sync report to target user
                await manager.broadcast_to_users(json.dumps({
                ...
                # V15 UI Sync: Refresh Admin list to show new tick
                # We reuse the logic below to broadcast users to all admins
                user_list = []
                for sov_id, data in user_auth.users.items():
                    bal = manager.get_user_balance(sov_id)
                    user_list.append({
...
async def login(auth: UserLogin, request: Request):
    ...
    if not auth_limiter.is_allowed(client_ip):
        return {"status": "REJECTED", "reason": "RATE_LIMIT_EXCEEDED"}
    return user_auth.login_user(auth.email_phone, auth.password)
...
@app.get("/api/v15/media/stream/{file_id}")
async def stream_video_tunnel(file_id: str, request: Request):
    # Sovereign V15: Zero-Latency High-Speed Tunnel [A_113]
    # Optimized for HTTP Range Requests and Direct MP4 Streaming
    file_path = os.path.join(SOV_DNA.storage, file_id)
    if not os.path.exists(file_path):
        logger.warning(f"[A_113] STREAM_ERR: File not found: {file_id}")
    ...
    if not range_header:
        # Full file direct stream: Best for initial chunk loading
        return FileResponse(file_path, media_type="video/mp4")
        
    # Range handling for high-speed seeking [Sovereign Logic]
    try:
    ...
    # Use standard FileResponse with range headers for maximum efficiency
    return FileResponse(
        file_path, 
        status_code=206, 
        headers={
