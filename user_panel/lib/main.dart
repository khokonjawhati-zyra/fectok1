...
String getSovereignUrl(int index, List<Map<String, dynamic>> mediaLedger) {
  ...
  if (mediaLedger.isNotEmpty && index < mediaLedger.length) {
    ...
    if (url.startsWith('/stream') && !url.startsWith('http')) {
        ...
        url = url.replaceAll(ipRegex, currentHost).replaceAll('localhost', currentHost).replaceAll(':8080', ':5000');
    }
    return url;
  }
...
class _MainNavigationState extends State<MainNavigation> with WidgetsBindingObserver {
  ...
  bool isLegallyAuthorized = true; // A_106 Gating (Bypassed for Vault Connection)
  bool isGatingEnabled = false; // Global Switch (OFF)
 ...
 // A_111 Tracker
            ...
            final decoded = json.decode(message);
             if (decoded['status'] == 'QUANTUM_SYNC_CONNECTED') {
                ...
                if (mounted) {
                  ...
                  if (decoded['name'] != null || decoded['bio'] != null || decoded['profile_pic'] != null || decoded['is_verified'] != null) {
                    setState(() {
                      ...
                      if (decoded['is_verified'] != null) isVerified = decoded['is_verified'] == true;
                    });
                    _saveMeshIdentity(meshID, userName, bio: userBio, profilePic: serverProfilePic, verified: isVerified);
                  }
                }
                ...
                if (targetId == myId) {
                 ...
                 setState(() {
                   ...
                   // V15 RECOVERY PULSE: Rebuild Social Sets from Mesh Data
                   _savedVideosSet.clear();
                   _likedVideosSet.clear();
                   for (int i = 0; i < sovereignMedia.length; i++) {
                     final m = sovereignMedia[i];
                     final List? savedBy = m['saved_by'] is List ? m['saved_by'] : null;
                     final List? likedBy = m['liked_by'] is List ? m['liked_by'] : null;
                     
                     // Global Index Injection (Sovereign Offset)
                     final int globalIdx = testVideos.length + i;
                     
                     if (savedBy != null && savedBy.any((u) => u.toString().toUpperCase() == meshID.toUpperCase())) {
                       _savedVideosSet.add(globalIdx);
                     }
                     if (likedBy != null && likedBy.any((u) => u.toString().toUpperCase() == meshID.toUpperCase())) {
                       _likedVideosSet.add(globalIdx);
                     }
                   }
                 });
                 debugPrint("A_111: Media Ledger Synced & AI Ranked (${sovereignMedia.length} videos) | Saved: ${_savedVideosSet.length}");
              ...
              if (mounted) {
                if (decoded['status'] == 'AUTO_APPROVED') {
                  setState(() => isVerified = true);
                  _saveMeshIdentity(meshID, userName, verified: true);
                }
              ...
              if (mounted) {
                final bool finalVerified = decoded['status'] == 'APPROVED' || decoded['status'] == 'AUTO_APPROVED';
                setState(() => isVerified = finalVerified);
                _saveMeshIdentity(meshID, userName, verified: finalVerified);
    ...
    try {
      ...
      if (savedID != null && savedID.startsWith("SOV_")) {
        setState(() {
          ...
          isVerified = prefs.getBool('sovereign_is_verified') ?? false;
  ...
  Future<void> _saveMeshIdentity(String id, String name, {String? profession, String? bio, String? profilePic, bool? verified}) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      ...
      if (verified != null) await prefs.setBool('sovereign_is_verified', verified);
    } catch(e) {
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Text('@${item['uploader_name'] ?? item['uploader'] ?? 'mesh_node'}', style: const TextStyle(color: SovereignColors.cyan, fontSize: 12)),
                      if (item['is_verified'] == true) ...[
                        const SizedBox(width: 4),
                        const Icon(Icons.verified, color: SovereignColors.cyan, size: 12),
                      ],
                      const SizedBox(width: 8),
                      Text(
                        'ID: ${item['uploader']}',
                ...
                if (item['is_verified'] == true)
                  WidgetSpan(
                    ...
                    child: Padding(
                      ...
                      child: const Icon(Icons.verified, color: SovereignColors.cyan, size: 14),
                    ),
                  ),
                const TextSpan(text: ' '),
...
class _QuantumPostHubState extends State<QuantumPostHub> {
  ...
  Future<void> _publish() async {
    ...
    try {
      final String host = kIsWeb ? (Uri.base.host.isNotEmpty ? Uri.base.host : 'localhost') : globalSovereignHost;
      final uri = Uri.parse("http://$host:5000/upload");
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  if (_imageFile != null) {
                    try {
                      ...
                      if (response.statusCode == 200) {
                        ...
                        final String host = kIsWeb ? (Uri.base.host.isNotEmpty ? Uri.base.host : globalSovereignHost) : globalSovereignHost;
                        uploadedPicPath = "http://$host:5000/stream/$filename";
                      }
                    } catch (e) {
                  ...
                  widget.channel.sink.add(json.encode({
                    ...
                    "profile_pic": uploadedPicPath, // V15 Master Standard Key
                    "profile_pic_path": uploadedPicPath, // Legacy Pulse Sync
                    "timestamp": DateTime.now().toIso8601String()
                  }));
  ...
  void initState() {
    ...
    if (sUrl != null && sUrl.isNotEmpty) {
      ...
      if (resolvedUrl.startsWith('/stream') && !resolvedUrl.startsWith('http')) {
        resolvedUrl = 'http://$host:5000$resolvedUrl';
      } else {
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          if (widget.index < widget.mediaLedger.length && widget.mediaLedger[widget.index]['is_verified'] == true) ...[
                            const SizedBox(width: 4),
                            const Icon(Icons.verified, color: SovereignColors.cyan, size: 14),
                          ],
                        ],
